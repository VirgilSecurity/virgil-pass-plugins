...
class virgil_login_acp_module
{
	...
	public function admin_main ()
	{
		...
		if (isset ($_POST ['submit']))
		{
            ...
            $virgil_login_disable      = $request->variable ('virgil_login_disable', '');
            ...
            $virgil_login_redirect_url = !empty($virgil_login_redirect_url) ? $virgil_login_redirect_url : $config ['server_protocol'] . $config ['server_name'] . '/ucp.php?mode=login&token={{virgilToken}}';
            ...
            // Save configuration.
			$config->set ('virgil_login_disable', $virgil_login_disable);
		...
		$template->assign_vars (array (
            ...
            'VIRGIL_LOGIN_DISABLED' => $virgil_login_disable,
	...
	public function handle_callback ()
	{
		...
		if (!empty($token))
		{
			...
			if (empty ($config ['virgil_login_disable']))
			{
                ...
                {
                    $template->assign_vars(array(
                        'VIRGIL_LOGIN_WARNING' => true,
                        'VIRGIL_LOGIN_VALIDATION_MESSAGE' => 'Virgil token validation failed.'
                    ));
                    $error = true;
                }
                ...
                if(($userInfo = $virgil_auth_client->get_user_info_by_token($token)) == false)
                {
                    $template->assign_vars(array(
                        'VIRGIL_LOGIN_WARNING' => true,
                        'VIRGIL_LOGIN_VALIDATION_MESSAGE' => 'Virgil user was not found.'
                    ));
                    $error = true;
                }
                ...
                if(!$error)
                {
                    ...
                    } else
                    {
                        ...
                        // Update virgil_implemented status
                        $this->set_virgil_implemented($user_id);
